...
userRouter
  ...
  .get(jsonParser, (req, res, next) => {
    ...
    userService
      .getUser(req.app.get("db"), loginUser[0], loginUser[1])
      .then((result) => {
        res.json(result);
      })
      .catch(next);
...
async function checkUserExists(req, res, next) {
  try {
    ...
    if (!thing) {
      userService.insertUser(req.app.get("db"), loginUser).then((result) => {
        const user = {
          ...loginUser,
          wins: 0,
          user_image: "goku",
...
userRouter
  ...
  .patch(jsonParser, (req, res, next) => {
    ...
    userService
      .updateUser(req.app.get("db"), userStats)
      .then((result) => {
  ...
  .delete(jsonParser, (req, res, next) => {
    ...
    userService
      .deleteUser(req.app.get("db"), userStats.id)
      .then((result) => {
